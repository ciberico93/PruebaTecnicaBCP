function listaProducto() {
    ...
    $('.dataTables_filter input').on('input', function () {
        ...
        if (filtrarPorDescripcion) {
            ...
            table.column(2).search(valorBusqueda).draw(); // Columna 2 es Descripción
        } else {
...
function EditarProducto(id) {
    var url = '/Producto/FrmEditarProducto' + '/?id=' + id;
...
function EliminarProducto(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.post('/Producto/EliminarProducto', { id }, function (data) {
                console.log(data);
            });
            Swal.fire(
                {
